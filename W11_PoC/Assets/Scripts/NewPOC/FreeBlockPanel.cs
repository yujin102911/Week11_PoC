...
public class FreeBlockPanel : MonoBehaviour
{
    ...
    private void Update()
    {
        _storageTxt.text = $"용량({RemainingBlockCount}/{Maxstorage})";
    }

    private void OnEnable()
    {
        //최초 1회 스폰
        if (!Is_spawn && _blockDatabases.Length != 0)
        {
    ...
    {
        if(_blockDatabases[stageIndex] != null)
    ...
    {
        if(RemainingBlockCount >= Maxstorage)
        {
            Debug.Log(this.name + "에 공간없음!!");
            return false;
        }
    ...
    /// 제거한 블럭 복구
    /// </summary>
    public void RecoverBlock(BlockItem block)
    {
    ...
    public BlockItem CreateBlockItemReturned(BlockData blockData, List<Vector2Int> currentShape, Vector2 position)
    {
        if (blockItemPrefab == null || blockData == null) return null;
        ...
        // BlockData와 회전된 Shape 설정
        newBlock.SetupFromPlacedInfo(blockData, currentShape);
        newBlock.SlotIndex = -1; // 슬롯에 속하지 않음
        newBlock.name = $"Block_Returned_{blockData.blockName ?? "Unknown"}";
